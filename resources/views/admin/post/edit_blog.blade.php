                        ...
                        <div class="row mg-b-25 mb-5">
                                ...
                                <div class="form-group mg-b-10-force">
                                    ...
                                    <select class="form-control select2" data-placeholder="Choose Category" name="category_id">
                                        ...
                                        @foreach($category as $row)
                                            <option value="{{$row->id}}" @if($post->category_id==$row->id) selected @endif>{{$row->category_name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div><!-- col-4 -->
                            ...
                            <div class="col-lg-4">
                                ...
                                <label class="custom-file">
                                    ...
                                    <img src="{{asset($post->post_img)}}" id="one"  class="mt-2" width="80px" height="80">
                                    <input type="hidden" name="oldImg_1" value="{{$post->post_img}}">
                                </label>
                            </div>
                        </div><!-- row -->
                        <hr>
                        <div class="form-group mt-3">
                            ...
                            <div class="form-check-inline">
                                <label class="form-check-label">
                                    <input type="radio" value="1" class="form-check-input" @if($post->status==1) checked @endif name="status">Publish
                                </label>
                            </div>
                            <div class="form-check-inline">
                                <label class="form-check-label">
                                    <input type="radio" value="0" class="form-check-input" @if($post->status==0) checked @endif name="status">Hide
                                </label>
                            </div>
                        </div>
